...
- 成功返回里重点看 `fact`、`ruleProjection`、`assetPrecipitation` 和 `projectionRefresh`；`projectionRefresh.status` 会说明护照/手册/交换投影是 `refreshed` 还是 raw 候选态。传入 `--evidence` 时，证据会保存在 fact payload，并在管理后台事实账本显示为 `evidenceSummary`。
